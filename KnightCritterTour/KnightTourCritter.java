...
public class KnightTourCritter extends KnightCritter
{
    ...
    public Location getBestMove( ArrayList<Location> locs )
    {
        ...
        for (int i = 0; i < locs.size(); i++) {
            if (accessGrid [locs.get(i).getRow()][locs.get(i).getCol()] < 0) {
                continue;
            }
            if (accessGrid [locs.get(i).getRow()][locs.get(i).getCol()] < min) {
                bestMove = locs.get(i);
                min = accessGrid[locs.get(i).getRow()][locs.get(i).getCol()];
            }
        }
    ...
    public void updateAccess( Location loc )
    {
        ...
        for (int i = 0; i < locs.size(); i++) {
            ...
            accessGrid [locs.get(i).getRow()][locs.get(i).getCol()] =
            accessGrid [locs.get(i).getRow()][locs.get(i).getCol()] - 1;
        }
    }
    ...
    public Location selectMoveLocation( ArrayList<Location> locs )
    {
        ...
        Location bestMove = getBestMove(locs);
        if (null != bestMove) {
            updateAccess(bestMove);
        }
        return bestMove;
    }
}
