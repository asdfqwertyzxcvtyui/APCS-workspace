...
public class JMCh12Exercises
{
    ...
    {
        char[] myArray = {'r','r','r','p','p','p','p','p','s','s','s','s','s','s'};
        Random myRand = new Random();
        return myArray[Math.abs(myRand.nextInt()) % myArray.length]; 
    }
    ...
    public String whatDoesMysteryCountCount()
    {
        ...
        String answer = mCount + " Returns the number of zeros in integer array up to any other number"; 
    ...
    public static int[] makePythagoreanTriple( int m, int n )
    {
        ...
        int[] myArray = new int[]{ (int)(Math.pow(m , 2) - Math.pow((double)n ,2)), 2 * m * n, (int)(Math.pow(m , 2) + Math.pow((double)n ,2))};
        return myArray;  // TODO fix this!!
    }
    ...
    public static int computeScore( String word )
    {
        ...
        for (int i = 0; i < s.length(); i++ ) {
            result = result + letterScores[s.charAt(i) - 'A'];
        }  
        return result;
    }
    ...
    public boolean[] prob11abcdeTrueFalse()
    {
        // return a boolean array with answers corresponding
        // to question (a) - (e)
        boolean[] myBool = {true, true, true, false, true};
        return myBool;  
    }
    ...
    {
        int count = 1;
        ArrayList<String> reverse = new ArrayList<>(list.size());
        for (int i = 0; i < list.size(); i++) {
            reverse.add(list.get(list.size() - count));
            count++;
        } 
        return reverse;  // TODO fix this!!
    }
    ...
    public boolean isMedian( double[] sample, double m )
    {
        ...
        if(contains == true) {
            while (sample[count1] != m) {
                count1++;
            }
            for (int j = count1 + 1; j < sample.length; j++) {
                count2++;
            }
    ...
    public static double averageTopTwo( int[] scores )
    {
        ...
        return (scores[iMax1] + scores[iMax2]) / 2; //_________________________________
    }
