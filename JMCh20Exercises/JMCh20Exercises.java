...
public class JMCh20Exercises<E>
{
    ...
    {
        if (null == head) {
            throw new NoSuchElementException("Original list is empty");
        }
        ListNode<E> rtn = head.getNext();
        head.setNext(null);
        return rtn;
    }
    ...
    public ListNode<String> concatenateStrings( ListNode<String> head )
    {
        ...
        while (current != null) {
            ...
            ListNode<String> item = new ListNode<String> (s, null);
            current = current.getNext();
            if (null == newHead) {
                newHead = item;
                prev = item;
    ...
    public ListNode<String> insertInOrder( ListNode<String> head, String s )
    {
        ...
        ListNode<String> current = head;
        ListNode<String> prev = null;
        ListNode<String> newHead = head;
        boolean biggest = false;
        while (null != current) {
            int value = s.compareTo(current.getValue());
            if (value < 0) {
                ListNode<String> item = new ListNode<String>(s, current);
                if (prev == null) {
                    newHead = item;
                }
                else {
    ...
    {
        ListNode<E> turtle = head;
        ListNode<E> rabbit = head;
        while (null != rabbit && (null != (rabbit = rabbit.getNext()))) {
    ...
    /**
     * Takes a list of strings referred to by head and moves all the strings that
     * match pattern to the end of the list, preserving their order. 
     * @param str Given String
     * @param pattern Pattern to match with nodes in the list
    ...
    {
        ListNode<String> outerCurr = head;
        while (null != outerCurr) {

            ListNode<String> min = outerCurr;
            ListNode<String> innerCurr = outerCurr.getNext();
            while (null != innerCurr) {
                if (innerCurr.getValue().compareTo(min.getValue()) < 0) {
                    min = innerCurr;
                }
                innerCurr = innerCurr.getNext();
            }
            if(!outerCurr.getValue().equals(min.getValue())) {
                String temp = outerCurr.getValue();
                outerCurr.setValue(min.getValue());
                min.setValue(temp);         
            }
            outerCurr = outerCurr.getNext();
        }
        return;
    }
